class Listeners {
    ...
    #drawLine(x0, y0, x1, y1, value) {
        ...
        let y = y0;
        while (true) {
            if (x >= 0 && x < this.grid.cells[0].length && y >= 0 && y < this.grid.cells.length) {
                this.grid.cells[y][x] = value;
            }
            if (x === x1 && y === y1) break;
            const e2 = 2 * err;
            if (e2 > -dy) {
        ...
        canvas.addEventListener('mousemove', (e) => {
            ...
            if (this.#isEditing) {
                if (this.#prevX !== null && this.#prevY !== null) {
                    const value = this.#isMouseDown ? 1 : (this.#isRightMouseDown ? 0 : null);
                    if (value !== null) {
                        this.#drawLine(this.#prevX, this.#prevY, this.#posX, this.#posY, value);
    ...
    sequenceSlider() {
        ...
        slider.steps = 1
        const canvasWidth = Number(Math.min(Math.floor(window.innerWidth - (window.innerWidth / 2)), canvas.width))
