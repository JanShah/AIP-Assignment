class Visualiser {
    ...
    constructor(ctx, grid) {
        this.agentScanDotSize = 0.2
        this.agentNeighborDotSize = 0.2
        this.agentVisitedDotSize = 0.2
        this.ctx = ctx
        this.grid = grid
        this.visitedPath = []
        ...
        this.draw();
        this.#overlays = new DocumentFragment();
    }

    setScreenOverlays(checked) {

        if(!this.notify({target:'getCurrentSolver'})) return
        this.isOverlaying = checked;
        this.notify({ target: "updateScanDotSize", value: 0 })
        this.notify({ target: 'updateAutoSolve', value: true })
        this.notify({ target: 'updateQuickSolve', value: true })
        this.agentScanDotSize = 0;
        this.agentVisitedDotSize = 0.3
        this.agentNeighborDotSize = 0.3;
        this.showRobotScan
        this.quickSolve = true;
        this.notify({target:'runSolver'})
    ...
    screenOverlays() {
        ...
        const screenShot = this.grid.getImage();
        const image = new Image();
        image.title = overlays.childNodes.length + 1;
        ...
        thumbContainer.style.minWidth = `${(overlays.childNodes.length + 1) * 100}px`
        thumbContainer.appendChild(imageClone);
        imageClone.addEventListener('mouseover', () => image.classList.add('highlight'))
        imageClone.addEventListener('mouseout', () => image.classList.remove('highlight'))
        imageClone.addEventListener('click', ()=>{
            overlays.removeChild(overlay)
            thumbContainer.removeChild(imageClone);
            if(thumbContainer.childNodes.length === 0){
                container.classList.add('hidden')
            }
        })
        image.addEventListener('mouseover',()=>{
            Array.from(overlays.childNodes).forEach((child)=> child.children[0].style.opacity = 1)
        });
        image.addEventListener('mouseout',()=>{
            Array.from(overlays.childNodes).forEach((child)=> child.children[0].style.opacity = null)
        });
    }
